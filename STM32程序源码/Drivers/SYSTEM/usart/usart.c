...
const char hex_1_Array[] = {0xfe};
uint8_t hex_Return_True_Array[] = {0x3F, 0xFF, 0xFE};
uint8_t hex_Return_Wrong_Array[] = {0x3F, 0xFE, 0xFE};
...
if (g_usart_rx_sta & 0x8000)        /* 接收到了数据? */
        {
					...
					char tmp[20]={0};
					sprintf(tmp,"%X %X %X",g_usart_rx_buf[0],g_usart_rx_buf[1],g_usart_rx_buf[2]);
					lcd_show_string(30, 200,200,16, len, tmp, BLUE);
            char *result = strstr((const char*)g_usart_rx_buf,hex_1_Array);
             
            if (result != NULL) {
							if(result[1] == 0){
				  CAL_GPIO3_Stepping_Motor_Forward(2,90);
					HAL_Delay(10000);
					CAL_GPIO3_Stepping_Motor_UnForward(2,115);
					HAL_Delay(200);
				};
							if(result[1] == 1){
				  CAL_GPIO4_Stepping_Motor_Forward(2,135);
					HAL_Delay(10000);
					CAL_GPIO4_Stepping_Motor_UnForward(2,135);
					HAL_Delay(200);
				};
              if(result[1] == 2){
				  ...
				  CAL_GPIO2_Stepping_Motor_Forward(2,90);
					...
					HAL_Delay(200);
							...
							if(result[1] == 4){
					CAL_GPIO1_Stepping_Motor_Forward(2,90);
					CAL_GPIO2_Stepping_Motor_Forward(2,100);
					CAL_GPIO3_Stepping_Motor_Forward(2,90);
				  CAL_GPIO4_Stepping_Motor_Forward(2,135);
					HAL_Delay(10000);
					CAL_GPIO1_Stepping_Motor_UnForward(2,105);
